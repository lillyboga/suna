...
| **self-host** | one-shot `migrate` from the image (below), or `KORTIX_AUTO_MIGRATE=1` at boot | before/with the app |
...
In docker-compose, make this a `migrate` service the `api` service `depends_on`. If you run a **single** API replica against your own database and want zero-config, set `KORTIX_AUTO_MIGRATE=1` and the app applies pending migrations at boot instead. (Don't use that with multiple replicas.) Prerequisite: your database must already have the Supabase Auth + Basejump objects the schema references.
